...
class ParserEngine:
    ...
    def parse_inquirer(self) -> InquirerInfo:
        return InquirerInfo(
            ...
            consultation_date=self.extract_field(
                r"fecha:?[\s\n]+(\d{2}/\d{2}/\d{4})"),
            consultation_time=self.extract_field(
                r"hora:?[\s\n]+(\d{2}:\d{2}(?::\d{2})?\s*(?:am|pm)?)")
        )
    ...
    def parse_personal_data(self) -> PersonalInfo:
        ...
        addr_match = re.search(
            r"direcciones:?\s*(.*?)(?=\stransunion|resumen|detalle|indagaciones|puntuacion|$)", self.text, re.DOTALL)
        addresses = []
        if addr_match:
            # Split by bullet point or newline
            raw_addr = re.split(r"[\n\*]|•", addr_match.group(1))
            addresses = [a.strip()
                         for a in raw_addr if a.strip() and len(a.strip()) > 5]

        # Robust extraction: match value until next known label
        labels = ["apellidos", "fecha nacimiento", "edad", "ocupacion",
                  "lugar nacimiento", "pasaporte", "estado civil", "telefonos", "direcciones"]
    ...
    def parse_score(self) -> Optional[CreditScore]:
        ...
        score_val = int(
            score_val_str) if score_val_str and score_val_str.isdigit() else None

        if score_val is None:
            return None

        factors_match = re.search(
            r"factores:?\s*(.*?)(?=\srnc|transunion|$)", self.text, re.DOTALL)
        factors = []
        if factors_match:
            factors = [f.strip() for f in factors_match.group(
                1).replace('*', '').split('\n') if f.strip()]
    ...
    def parse_account_summaries(self) -> List[AccountSummary]:
        section_match = re.search(
            r"resumen de cuentas abiertas:?\s*(.*?)(?=\sleyenda|detalle|indagaciones|transunion|$)", self.text, re.DOTALL)
        ...
        try:
            ...
            for i in range(0, len(data_chunk), 11):
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to parse account summary row: {row}. Error: {e}")
        except Exception as e:
    ...
    def parse_account_details(self) -> List[AccountDetail]:
        section_match = re.search(
            r"detalle de cuentas abiertas\s*(.*?)(?=\sdetalle de cuentas cerradas|indagaciones|$)", self.text, re.DOTALL)
        if not section_match:
            return []
        ...
        for block in blocks:
            try:
                ...
                logger.warning(
                    f"Failed to parse account block: {block[:100]}... Error: {e}")
    ...
    def parse_account_details_v2(self) -> List[AccountDetail]:
        ...
        section_match = re.search(
            r"detalle de cuentas abiertas\s*(.*?)(?=\sdetalle de cuentas cerradas|totales generales|indagaciones|$)", self.text, re.DOTALL)
        if not section_match:
            return []
        ...
        for i in range(len(headers)):
            ...
            end_pos = headers[i+1].start() if i + \
                1 < len(headers) else len(section_text)
            ...
            try:
                ...
                for record_content in sub_records:
                    ...
                    wrap_match = re.search(
                        rf"(.*?)(?=\s*(?:{status_kw_pat}|{date_pat}))", record_content, re.DOTALL | re.I)
                    ...
                    detail = self._extract_detail_from_content(
                        current_acc_type, current_sub, remaining_record)
                    if detail:
                        details.append(detail)
            except Exception as e:
                logger.warning(
                    f"Failed to parse account block starting with {header_line}: {e}")
                import traceback
    ...
    def _extract_detail_from_content(self, acc_type: str, subscriber: str, content: str) -> Optional[AccountDetail]:
        ...
        if vector_match:
            ...
            content = content[:vector_match.start()] + \
                content[vector_match.end():]
        ...
        currency = "DOP"
        if re.search(r"usd|us\$", content, re.I):
            currency = "USD"
        content = re.sub(r"dop|usd|rd\$|us\$", " ", content, flags=re.I)

        # 4. Modality (Usually looks like "12/36 MENSUAL" or similar)
        # Often contains a slash but not a full date
        modality_match = re.search(
            r"(\d+/\d+\s+[a-z]+|\b(mensual|quincenal|semanal|anual)\b)", content, re.I)
        ...
        status_kw = ["al dia", "vigente", "cancelada", "atraso",
                     "proceso judicial", "reestructurado", "legal"]
        status = "unknown"
        for kw in status_kw:
        ...
        monetary_values = re.findall(
            r"(\d{1,3}(?:,\d{3})*(?:\.\d+)?|0\.00)", content)
        # Filter out stray single digits if we have better candidates
        monetary_values = [v for v in monetary_values if len(
            v.replace(',', '').split('.')[0]) > 0]

        credit_limit = self._parse_money(
            monetary_values[0]) if len(monetary_values) > 0 else 0.0
        current_balance = self._parse_money(
            monetary_values[1]) if len(monetary_values) > 1 else 0.0
        balance_in_arrears = self._parse_money(
            monetary_values[2]) if len(monetary_values) > 2 else 0.0
        min_pmt = self._parse_money(monetary_values[3]) if len(
            monetary_values) > 3 else 0.0
