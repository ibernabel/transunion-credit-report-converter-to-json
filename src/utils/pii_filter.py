...
class PIIFilter(logging.Filter):
    ...
    # More robust phone pattern (DOM inclusive)
    PHONE_PATTERN = re.compile(
        r'(\+?1?\s*)?(\(?\d{3}\)?[\s.-]?)?\d{3}[\s.-]?\d{4}|\b(809|829|849)\d{7}\b')
    EMAIL_PATTERN = re.compile(
        r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b')
    CREDIT_CARD_PATTERN = re.compile(r'\b\s*(?:\d[ -]?){13,16}\b')
    # Account number pattern (Generic 10+ digits)
    ...
    def _redact_pii(self, text: str) -> str:
        ...
        # Redact general account numbers
